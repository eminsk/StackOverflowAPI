...
Features score badges, answer count pills, teaser excerpts, author reputation,
active selection highlighting, and bookmark indicators.
"""
...
from src.utils.highlighter import extract_text_preview
from src.utils.bookmarks import BookmarkManager
from src.ui.theme import (
    SO_ORANGE, COLOR_SUCCESS, COLOR_PRIMARY, COLOR_DANGER, COLOR_BOOKMARK,
...
class QuestionCard(ctk.CTkFrame):
    ...
    Displays:
    - Score badge (upvotes)
    - Answer count pill with solved indicator
    - Views counter
    - Creation date
    - Question title (bold, clean)
    - 1-2 line body preview excerpt
    - Tag chips
    - Author name & reputation
    - Active selection state with SO orange border & accent
    """
    ...
    def __init__(
        ...
        on_click_callback: Callable[[int, 'QuestionCard'], None],
        **kwargs
    ):
    ...
    def setup_ui(self):
        ...
        body_html = self.question_data.get("body", "")
        ...
        # Formatting Views (e.g. 1.5k, 2.4M)
        ...
        # 1. Top Meta Row: Score, Answers, Views, Bookmark Star, Date
        self.top_row = ctk.CTkFrame(self, fg_color="transparent")
        self.top_row.pack(fill="x", padx=14, pady=(12, 4))

        # Score Badge
        score_prefix = "▲ " if score >= 0 else "▼ "
        score_color = COLOR_SUCCESS if score > 0 else (COLOR_TEXT_MUTED if score == 0 else COLOR_DANGER)
        ...
        self.score_badge.pack(side="left", padx=(0, 8))
        ...
        self.ans_pill.pack(side="left", padx=(0, 8))
        ...
        if self.bookmark_mgr.is_bookmarked(self.question_id):
            self.bm_star = ctk.CTkLabel(
                self.top_row,
                text="⭐",
                font=ctk.CTkFont(size=11),
                text_color=COLOR_BOOKMARK
            )
            self.bm_star.pack(side="left", padx=(0, 4))
        ...
        self.title_label.pack(fill="x", padx=14, pady=(2, 4))

        # 3. Teaser Preview (Clean 1-line snippet of body)
        preview_text = extract_text_preview(body_html, max_chars=110)
        if preview_text:
            ...
            self.preview_label.pack(fill="x", padx=14, pady=(0, 6))

        # 4. Bottom Row: Tags & Author
        self.bottom_row = ctk.CTkFrame(self, fg_color="transparent")
        self.bottom_row.pack(fill="x", padx=14, pady=(2, 12))
    ...
    def bind_events(self):
        """Bind hover and click events to card and all nested children."""
    ...
    def set_selected(self, selected: bool):
        """Highlight card with active SO accent border and selected surface."""
    ...
    def on_card_click(self, event=None):
        if self.on_click_callback:
            self.on_click_callback(self.question_id, self)
