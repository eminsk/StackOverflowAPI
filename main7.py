...
class StackOverflowGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        self.question_details_browser.setOpenExternalLinks(False)  # Handle manually
        self.question_details_browser.anchorClicked.connect(self.handle_external_link)

        self.answers_browser = QTextBrowser()
        self.answers_browser.setOpenExternalLinks(True)  # Allow external links in answers
    ...
    def handle_external_link(self, url):
        """Handle link clicks in question details browser"""
        ...
        # If it's an internal link (like code blocks), ignore
        else:
            pass
    ...
    def load_question(self, url, language):
        ...
        <head>
            <style>
                ...
                .external-link {{ display: inline-block; margin-top: 15px; padding: 10px 20px; background: #28a745; color: white; text-decoration: none; border-radius: 5px; font-weight: bold; }}
                .external-link:hover {{ background: #218838; }}
            </style>
        </head>
        <body>
